...
namespace CouponManager.Api.Controllers
{
    ...
    public class CouponsController : ControllerBase
    {
        ...
        [HttpPut]
        public async Task<IActionResult> PutAsync(CouponViewModel model)
        {
            var userId = User.Claims.Where(c => c.Type == "UserId").Select(c => c.Value).SingleOrDefault();
            await _couponService.AddAsync(model, userId, _cancellationToken);
            return NoContent();
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteAsync(CouponDeleteViewModel model)
        {
            var userId = User.Claims.Where(c => c.Type == "UserId").Select(c => c.Value).SingleOrDefault();
            await _couponService.DeleteAsync(model, userId, _cancellationToken);
            return NoContent();
        }
    }
}
